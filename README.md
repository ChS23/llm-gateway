...
Production-ready LLM gateway на Rust. Принимает OpenAI-совместимые запросы, сам разбирается куда их отправить — с балансировкой, failover, guardrails и полной наблюдаемостью.
...
Body Limit (1MB)
  │
Auth ── sha256(key) → postgres, scope check, Redis rate limit
  │
Guardrails ── RegexSet single-pass: 6 injection + 4 secret паттернов
  │
Router ── resolve(model) → [провайдеры] → стратегия → выбор
  │             ↑
  │       Circuit Breaker фильтрует упавших
  │
Provider ── reqwest → LLM API
  │
Response ── JSON или SSE proxy с inline TTFT/TPOT
  │
OTel ── span → Langfuse, метрики → Prometheus
```
...
**Circuit breaker:** 5 подряд ошибок → Open → 30s cooldown → HalfOpen → 3 probe → Closed.
